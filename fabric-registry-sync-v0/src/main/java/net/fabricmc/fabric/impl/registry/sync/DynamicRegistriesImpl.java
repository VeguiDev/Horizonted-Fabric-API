...
public final class DynamicRegistriesImpl {
	...
	private static final List<RegistryDataLoader.RegistryData<?>> SYNCED_REGISTRIES = new ArrayList<>(RegistryDataLoader.SYNCHRONIZED_REGISTRIES);
	...
	static {
		DYNAMIC_REGISTRIES.addAll(RegistryDataLoader.WORLDGEN_REGISTRIES);
		DYNAMIC_REGISTRIES.addAll(RegistryDataLoader.DIMENSION_REGISTRIES);

		for (RegistryDataLoader.RegistryData<?> vanillaEntry : DYNAMIC_REGISTRIES) {
	...
	public static <T> void addSyncedRegistry(ResourceKey<? extends Registry<T>> key, Codec<T> clientCodec, DynamicRegistries.SyncOption... options) {
		...
		SYNCED_REGISTRIES.add(new RegistryDataLoader.RegistryData<>(key, clientCodec, false));
	...
	public static @Unmodifiable List<RegistryDataLoader.RegistryData<?>> getSyncedRegistries() {
		return List.copyOf(SYNCED_REGISTRIES);
	}
}
