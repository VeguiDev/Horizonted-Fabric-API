...
public class PayloadTypeRegistryImpl<B extends FriendlyByteBuf> implements PayloadTypeRegistry<B> {
	...
	public <T extends CustomPacketPayload> CustomPacketPayload.TypeAndCodec<? super B, T> registerLarge(CustomPacketPayload.Type<T> id, StreamCodec<? super B, T> codec, int maxPayloadSize) {
		...
		// Defines max packet size, increased by length of packet's ResourceLocation to cover full size of CustomPacketPayloadX2YPackets.
		int ResourceLocationSize = ByteBufUtil.utf8MaxBytes(id.id().toString());
		int maxPacketSize = maxPayloadSize + VarInt.getByteSize(ResourceLocationSize) + ResourceLocationSize + 5 * 2;
