...
public final class ServerConfigurationNetworking {
	 ...
	 * Use {@link #unregisterReceiver(ServerConfigurationPacketListenerImpl, net.minecraft.resources.ResourceLocation)} to unregister the existing handler.
	 *
	 ...
	 * @see ServerConfigurationNetworking#unregisterGlobalReceiver(net.minecraft.resources.ResourceLocation)
	 * @see ServerConfigurationNetworking#registerReceiver(ServerConfigurationPacketListenerImpl, net.minecraft.network.protocol.common.custom.CustomPacketPayload.Type, ConfigurationPacketHandler)
	 */
	 ...
	 * or it was not registered using {@link #registerGlobalReceiver(net.minecraft.network.protocol.common.custom.CustomPacketPayload.Type, ConfigurationPacketHandler)}
	 * @see ServerConfigurationNetworking#registerGlobalReceiver(net.minecraft.network.protocol.common.custom.CustomPacketPayload.Type, ConfigurationPacketHandler)
	...
	/**
	 * Registers a handler for a payload type.
	 * This method differs from {@link ServerConfigurationNetworking#registerGlobalReceiver(net.minecraft.network.protocol.common.custom.CustomPacketPayload.Type, ConfigurationPacketHandler)} since
	 ...
	 * @return the previous handler, or {@code null} if no handler was bound to the channel,
	 * or it was not registered using {@link #registerReceiver(ServerConfigurationPacketListenerImpl, net.minecraft.network.protocol.common.custom.CustomPacketPayload.Type, ConfigurationPacketHandler)}
	 */
	@Nullable
	 ...
	 *
	 * @param handler the network handler
	 * @return all payload ids that the connected client declared it can receive during configuration
	 */
	 ...
	 *
	 * @param handler the network handler
	 * @param type the payload id
	 * @return {@code true} if the connected client has declared the ability to receive a specific type of packet
	 */
	public static boolean canSend(ServerConfigurationPacketListenerImpl handler, CustomPacketPayload.Type<?> type) {
		Objects.requireNonNull(handler, "Server configuration network handler cannot be null");
	...
	public static Packet<ClientCommonPacketListener> createS2CPacket(CustomPacketPayload payload) {
		...
		Objects.requireNonNull(payload.type(), "CustomPayload#type() cannot return null for payload class: " + payload.getClass());
	...
	public interface ConfigurationPacketHandler<T extends CustomPacketPayload> {
		 ...
		 * Modification to the game should be {@linkplain net.minecraft.server.MinecraftServer#submit(Runnable) scheduled} using the Minecraft server instance from {@link ServerConfigurationNetworking#getServer(ServerConfigurationPacketListenerImpl)}.
		 *
		 * <p>An example usage of this:
		 * <pre>{@code
		 * // use PayloadTypeRegistry for registering the payload
		 * ServerConfigurationNetworking.registerReceiver(handler, BOOM_PACKET_TYPE, (payload, context) -> {
		 *
		 * });
		 * }</pre>
		 *
